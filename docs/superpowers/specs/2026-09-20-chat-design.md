...
- 频道配色（2026-09-20 用户指定色值）：区域 `#363A3C` 深灰、私聊 `#058306` 绿、公会 `#5991CA` 蓝（预留）、队伍 褐色 `#8B4513`（预留；用户只给「褐色」，取标准 saddle brown，可调）；世界窗内消息用默认墨色。详见 `docs/聊天系统设计.md`。
- 频道配色变更（2026-09-21 用户指定）：世界聊颜色由默认墨色改为与区域聊一致的 `#363A3C`。
...
- 消息格式：区域/世界 `名字：内容`；私聊用用户指定模板——发送方视角 `你 对 {目标名} 说: {内容}`、接收方视角 `{发送名} 对 你 说: {内容}`；其中角色名带下划线（`.name` 类），「你」字用 `#F52627` 红；前端按结构化字段（senderName/targetName/content）拼装渲染，不落预拼字符串；均带 time 前缀
